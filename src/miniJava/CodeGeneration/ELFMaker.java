...
public class ELFMaker {
	...
	public ELFMaker(ErrorReporter errors, long textSize, long bssSize) {
		...
		//flags: 0000 0110
		//last section: n/a, executable, should exist in memory, not writeable
		//I don't think it needs to have null terminated strings?
		text.sh_flags = 0b00000110; 
		text.sh_type = 0x1; // Program data
		...
		bss.data = null;
		bss.sh_size = bssSize;
		bss.sh_type = 0x8; // 0x8: no bits (no program data)
		//flags: exist in memory, be writeable, not executable
		//so 0011
		bss.sh_flags = 0b0011; // TODO: what are the flags of the bss section?
		sections.add( bss );
		
		// make .shstrtab
		shstrtab.sectionName = ".shstrtab";
		shstrtab.sh_type = 0x3; // strtab section
		//flags: has null terminated strings, doesn't exist in memory, doesn't need to be executed
		//so just 0x20
		shstrtab.sh_flags = 0x20; // TODO: what are the flags of this section?
	 ...
	 * @param fname
	 * @param textSection
	 * @param entrypoint  offset from start of text section
	 */
	public void outputELF(String fname, byte[] textSection, long entrypoint) {
		...
		phdr.p_type = 0x6; // program header
		phdr.p_flags = 0b000; // I don't think it needs to be readable, writeable, *or* executable?
		...
		textSeg.p_type = 0x1; // text has the program in it, so I guess it must be loaded
		textSeg.p_flags = 0b101; // text must be executable and readable
		...
		try {
			...
			_errors.reportError(new CompilerError("FileNotFoundException: " + e));
		} catch (IOException e) {
			_errors.reportError(new CompilerError("IOException: " + e));
		}
	}

	public void saveInstructions(String fname,byte[] textSection, long entrypoint){
		//save instructions in plaintext hex for disassembly
		try {
			FileOutputStream f = new FileOutputStream(fname);
			// String firstLine = "Size: " + textSection.length + "; entrypoint is at byte " + entrypoint;
			// f.write(firstLine.getBytes());
			// f.write("\n".getBytes());
			for (byte text : textSection){
				f.write(byteToHex(text).getBytes()); //byte -> string hex -> bytes of that string lmao
				f.write("\n".getBytes());
			}
			f.close();
			System.out.println("Saved instructions to file " + fname);
		} catch( FileNotFoundException e) {
			_errors.reportError(new CompilerError("FileNotFoundException: " + e));
		} catch (IOException e) {
			_errors.reportError(new CompilerError("IOException: " + e));
	...
	private ELFSection makeNullSection() {
		...
		e.sh_name = 0;
		e.sh_type = 0; //what type is the null section? it's 0 lol
		e.sh_flags = 0;
		e.sh_addr = 0;
		e.sh_offset = 0;
		e.sh_size = 0;
