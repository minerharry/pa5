...
public class InstructionList {
	...
	public int addAll(InstructionList other){ //THIS DOES NOT PRESERVE DATA FROM THE ORIGINAL!
		int firstIdx = _currentIdx;
		for (Instruction i : other._instructions){
			add(i);
		}
		return firstIdx;
	}

	public int addAll(List<InstructionList> other){
		int firstIdx = _currentIdx;
		for (InstructionList il : other){
			addAll(il);
		}
		return firstIdx;
	}
	...
	public Instruction last(){
		return _instructions.get(_instructions.size()-1);
	}
